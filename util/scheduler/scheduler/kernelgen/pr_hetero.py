...
def to_epochkernel_funcname(kernel: PRKernels) -> str:
    func_names = {
        ...
        PRKernels.GPU_warp_min:   'epoch_pr_pull_gpu_warp_min',
        PRKernels.GPU_block_min:  'epoch_pr_pull_gpu_block_min',
    }
...
def ker_to_string(kernel: PRKernels) -> str:
    string_names = {
        ...
        PRKernels.GPU_warp_min:   'PR GPU warp-min',
        PRKernels.GPU_block_min:  'PR GPU block-min',
    }
...
{to_epochkernel_funcname(kernel.kerid)}<<<{kernel.block_count}, {kernel.thread_count}, 0, compute_streams[{idx}]>>>(
        ...
        cu_dists[{devid}], cu_updateds[{devid}], g.num_nodes, cu_degrees);
